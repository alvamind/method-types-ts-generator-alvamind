...
export const test10: TestCase = {
  ...
  output: {
    ...
    tsMorphAssertion: (project) => {
      const interfaceDeclaration = project.getSourceFileOrThrow('test10.d.ts')?.getInterface('ExposedMethods');
      ...
      if (!returnTypeText.match(/Promise<MyData<{ input: T; option: K; }>>/)) {
        return { passed: false, message: `Expected return type Promise<MyData<{ input: T; option: K; }>>, but found ${returnTypeText}` };
      }
